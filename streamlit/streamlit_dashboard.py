...
from explainer import  features_global
...
### Data with proba:
my_data = pd.read_csv("../Data/proba_final/data.csv")
   ...
   # image = Image.open('Sans titre.png')
    ...
    def feature_local():
        ...
        if user_choice == '**Analyse locale de motifs**':
            ...
            shap_values_test , explainer = explainer_model()
            shap.initjs() 

            plt.figure(figsize=(5,5))
            
            st_shap(shap.force_plot(explainer.expected_value[1], shap_values_test[1][instance_idx,:],
    ...
    def feature_global():
        ...
        if user_choice == '**Analyse globale de motifs**':
            ...
            features_global()
            #shap_values_test = explainer_model()
            # shap.initjs()
            # st_shap(shap.summary_plot(shap_values_test[0],
            #       features = test,
